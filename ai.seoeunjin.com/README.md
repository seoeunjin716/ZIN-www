...
```bash
# 개별 서비스 빌드
docker build -t authservice:latest ./ai.seoeunjin.com/authservice
docker build -t crawlerservice:latest ./ai.seoeunjin.com/crawlerservice
docker build -t chatbotservice:latest ./ai.seoeunjin.com/chatbotservice
...
1. `ai.seoeunjin.com/` 하위에 새 서비스 디렉토리 생성
2. `app/` 디렉토리 구조 생성
